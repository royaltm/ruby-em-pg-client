...
* fully async auto re-connects on connection loss (like server restarts)
...
* implement EM adapted version of get_copy_data and put_copy_data
...
* ruby >= 1.9 (tested: 1.9.1p378, 1.9.2-p280, 1.9.2-p320, 1.9.3-p194)
...
Because I didn't find ruby-pg's EM implementation to fit my needs.
...
The rest of above methods provides PG::Result. You need to +clear+ obtained
...
for EM-Synchrony or other implementation of Fiber untangled EventMachine.

The +require+ string is "em-synchrony/pg" instead of "pg/em".

Under +em-synchrony+ PG::EM::Client.new is fully asynchronous and blocks
only current fiber. This also applies to PG::EM::Client#reset.
