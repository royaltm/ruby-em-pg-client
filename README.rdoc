...
==== PG::Connection methods coverage
...
2. async / blocking methods (returning +Deferrable+ only when EM is running):
  ...
  - +exec+ (alias: +async_query+)
  - +prepare+
  - +exec_prepared+
  - +describe_prepared+
  - +describe_portal+

Outside EventMachine event loop theese are regular PG::Connection blocking methods.

All above methods (1 & 2) accept code block which is attached to callback and errback hooks
of returned +Deferrable+.

==== Special options
There are 3 additional connection options and 1 standard +pg+ option used by
async library (the additional ones are not passed to +pg+).
You may add them as one of the *hash options* to Client.new or Client.async_connect
or simply use accessor methods to change them on the fly.
...
accessor method only affects asynchronous methods.
