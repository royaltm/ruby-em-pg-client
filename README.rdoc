...
It is also possible to define +on_reconnect+ callback to be invoked
while the connection has been reset. It is called just before the send query
command is executed:
...
As you can see it is possible to send async query from inside on_reconnect
proc however you have to pass Deferrable from the async call back to it's
...
==== Async reconnect with on_reconnect callback
  ...
  EM.synchrony do
    pg = PG::EM::Client.new dbname: 'test'
    pg.prepare('bar', 'select * from foo order by cdate desc')
    pg.on_reconnect = proc do |c, e|
      c.prepare('bar', 'select * from foo order by cdate desc')
    end
    try_query = lambda do
      pg.exec_prepared('bar') do |result|
